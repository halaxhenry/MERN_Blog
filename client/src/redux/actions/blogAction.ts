...
async (dispatch: Dispatch<IAlertType | ICreateBlogsUserType>) => {
  ...
  try {
    ...
    const res: any = await postAPI('blog', newBlog, access_token)
...
async (dispatch: Dispatch<IAlertType | IGetHomeBlogsType>) => {
  try {
    ...
    const res: any = await getAPI('home/blogs')
...
async (dispatch: Dispatch<IAlertType>) => {
  ...
  try {
    ...
    const res: any = await putAPI(`blog/${newBlog._id}`, newBlog, access_token)
